...
exports.createEqdsPatient = createEqdsPatient;
exports.createPatientFromExample = createPatientFromExample;
exports.createTestPatient = createTestPatient;
exports.getOwnershipTotal = getOwnershipTotal;
exports.getAnimalInfo = getAnimalInfo;
exports.getColorInfo = getColorInfo;
exports.getBreedingInfo = getBreedingInfo;
 ...
 */
function createEqdsPatient(params) {
    ...
    if (params.owners && params.owners.length > 0) {
        ...
        if (!(0, validators_1.validateOwnershipPercentages)(totalPercentage)) {
    ...
    const animalExtension = {
        url: "http://hl7.org/fhir/StructureDefinition/patient-animal",
        extension: [
            {
                url: "species",
                valueCodeableConcept: {
                    coding: [types_1.HORSE_SPECIES_CODING]
                }
            },
            {
                url: "genderStatus",
                valueCodeableConcept: {
                    coding: [{
                            system: types_1.EQDS_URLS.CODESYSTEM_SEX,
                            code: params.genderStatus,
                            display: types_1.EquineSexCodes[params.genderStatus].display
                        }]
                }
            }
        ]
    ...
    if (params.breed) {
        animalExtension.extension.push({
            ...
            valueCodeableConcept: {
                coding: [{
                        system: types_1.EQDS_URLS.CODESYSTEM_BREED,
                        code: params.breed,
                        display: types_1.BreedCodes[params.breed].display
                    }]
            }
        });
    }
    // Start with required extensions
    const extensions = [animalExtension];
    // Add color extension if provided
    if (params.color) {
        const colorExtension = {
            url: types_1.EQDS_URLS.EXTENSION_COLOR,
            valueCodeableConcept: {
                coding: [{
                        system: types_1.EQDS_URLS.CODESYSTEM_COLOR,
                        code: params.color,
                        display: types_1.ColorCodes[params.color].display
                    }]
            }
        };
    ...
    if (params.owners && params.owners.length > 0) {
        params.owners.forEach((owner) => {
            const ownershipExtension = {
                url: types_1.EQDS_URLS.EXTENSION_OWNERSHIP,
                extension: [
                    {
                        url: "owner",
                        valueReference: {
    ...
    if (params.sire || params.dam) {
        ...
        const breedingInfoExtension = {
            url: types_1.EQDS_URLS.EXTENSION_BREEDING,
    ...
    const patient = {
        ...
        id: params.id,
        meta: {
            profile: [types_1.EQDS_URLS.PROFILE_PATIENT]
        },
        text: {
            status: "generated",
            div: `<div xmlns='http://www.w3.org/1999/xhtml'>${types_1.EquineSexCodes[params.genderStatus].display} '${params.name}'${params.breed ? `, ${types_1.BreedCodes[params.breed].display}` : ''}</div>`
        },
 ...
 */
function createPatientFromExample() {
 ...
 */
function createTestPatient(id, genderStatus, breed) {
    return createEqdsPatient({
        id,
 ...
 */
function getOwnershipTotal(patient) {
 ...
 */
function getAnimalInfo(patient) {
 ...
 */
function getColorInfo(patient) {
    const colorExt = patient.extension.find((ext) => ext.url === types_1.EQDS_URLS.EXTENSION_COLOR);
    return colorExt?.valueCodeableConcept.coding?.[0];
}
/**
 * Extract breeding information from patient
 */
function getBreedingInfo(patient) {
    const breedingExt = patient.extension.find((ext) => ext.url === types_1.EQDS_URLS.EXTENSION_BREEDING);
    if (!breedingExt) {
        return null;
    }
